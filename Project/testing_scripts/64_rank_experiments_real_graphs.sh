...
srun -N64 --ntasks-per-node=1 --overcommit $EXECUTABLE $SCRATCH/TestGraphs/email-enron.adj 64 > $SCRATCH/64_rank_results/64r-email.log &
srun -N64 --ntasks-per-node=1 --overcommit $EXECUTABLE $SCRATCH/TestGraphs/p2p-gnutella.adj 64 > $SCRATCH/64_rank_results/64r-p2p.log &
wait
